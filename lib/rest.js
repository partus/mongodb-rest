...
module.exports = function (app, config) {
    ...
    // app.use(urlPrefix+"/:db/*", function(req,res,next){
    //     console.log(req.params, req.originalUrl, req.url, req.path);
    //     // if(req.params.db == "collectionapi") req.params.db = "insight";
    //     req.originalUrl = req.originalUrl.replace("collectionapi", "insight");
    //     next();
    // })
    //
    ...
    */
    app.get(urlPrefix + '/:db/:collection/:id?', function(req, res) { 
        if(req.params.db == "collectionapi") {
            req.params.db = "insight";
        }
        ...
        if(req.body) {
            connection.connect(dbConnectionStr + '/' + req.params.db, function (err, db) {
                ...
                db.collection(req.params.collection, function(err, collection) {
                    ...
                    var upsert = hooks[req.params.db] 
                        && hooks[req.params.db][req.params.collection]
                        && hooks[req.params.db][req.params.collection].identifyBy && hasKeys(hooks[req.params.db][req.params.collection].identifyBy,doc)
                    if(upsert) {
